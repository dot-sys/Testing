...
$usnDump | Where-Object { 'RenameOldName', 'RenameNewName' -contains $_.'UpdateReasons' } | Sort-Object -Property 'UpdateTimestamp' -Descending | Group-Object "UpdateTimestamp" | Format-Table -AutoSize @{l = "Timestamp"; e = { $_.Name } }, @{l = "Old Name"; e = { Split-Path -Path $_.Group.'FilePath'[0] -Leaf } }, @{l = "New Name"; e = { Split-Path -Path $_.Group.'FilePath'[1] -Leaf } } | Out-File -FilePath Renamed_Files.txt -Append -Width 4096
$usnDump | Where-Object { $_.'Extension' -eq ".rpf" -and $_.'UpdateReasons' -match 'FileDelete|DataTruncation' } | Select-Object 'FilePath', 'UpdateTimestamp' | Sort-Object 'UpdateTimestamp' -Descending -Unique | Out-File Deletedrpf.txt -Append -Width 4096
$usnDump | Where-Object { $_.'Extension' -eq ".rar" -or $_.'Extension' -eq ".zip" -or $_.'Extension' -eq ".7z" } | Select-Object 'FilePath', 'UpdateTimestamp' | Sort-Object 'UpdateTimestamp' -Descending -Unique | Out-File Compressed.txt -Append -Width 4096
$usnDump | Where-Object { $_.'Extension' -eq ".bat" -and $_.'UpdateReasons' -match "RenameOldName|FileDelete" } | Select-Object 'FilePath', 'UpdateTimestamp' | Sort-Object 'UpdateTimestamp' -Descending -Unique | Out-File ModifiedBats.txt -Append -Width 4096
$usnDump | Where-Object { $_.'Extension' -eq ".exe" -and $_.'UpdateReasons' -match "ObjectIdChange" } | Select-Object 'FilePath', 'UpdateTimestamp' | Sort-Object 'UpdateTimestamp' -Descending -Unique | Out-File ObjectIDChange.txt -Append -Width 4096
$usnDump | Where-Object { $_.'UpdateReasons' -match "DataTruncation" -and $_.'Extension' -eq ".exe" -and $_.'Extension' -ne ".pf" } | Select-Object 'FilePath', 'UpdateTimestamp' | Sort-Object 'UpdateTimestamp' -Descending -Unique | Out-File ReplacedExe.txt -Append -Width 4096
